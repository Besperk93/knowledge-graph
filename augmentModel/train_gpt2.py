"""
Used to train the GPT2 model for comparison with our KnowGPT2 model, taken from the orignal paper's repository. Full credit must be given to the authors for this. Only a few small changes made in order to get the script to run on my machine.
...
from src.datasets.mathematica import MathematicaMathDataset
from src.datasets.mathematica_with_steps import MathematicaWithStepsMathDataset

# NOTE: Function to see where the memory issues are coming from
...
def run_training(args, train_data):
    ...
    if args.load:
        ...
        model = transformers.GPT2LMHeadModel.from_pretrained(args.arch, use_cache=False)
