...
def load_state(net, optimizer, scheduler, args, load_best=False):
    ...
    base_path = "./Vault/mtb/ouput/"
    amp_checkpoint = None
    checkpoint_path = os.path.join(base_path,"test_checkpoint.pth.tar")
    best_path = os.path.join(base_path,"test_model_best.pth.tar")
...
def load_results():
    ...
    losses_path = "./Vault/mtb/output/data/test_losses_per_epoch.pkl"
    accuracy_path = "./Vault/mtb/output/data/test_accuracy_per_epoch.pkl"
    if os.path.isfile(losses_path) and os.path.isfile(accuracy_path):
        losses_per_epoch = load_pickle("test_losses_per_epoch.pkl")
