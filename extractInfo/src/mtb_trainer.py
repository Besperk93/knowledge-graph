...
# NOTE: These are slightly different in the two training scripts
...
def train_and_fit(args):
    ...
    # NOTE:
    # From hf Trainer class args:
    #  -- Model handled in init
    #  -- dataset handled in init
    #  -- tokenizer handled in init
    #  -- Optimizers handled in init (use defaults anyway)
    #  -- Training Arguments get passed to train()
    #
    ...
    lower_case = True
    model_name = 'BERT'
    net = Model.from_pretrained(model, force_download=False,      model_size=model)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            # NOTE: This is where the MTB task is trained, it seems to be an adaption to the function in the default trainer train() function, lines: 1103-1583
            # NOTE: The actual training code lies within 1290-1583, prior code is setting configurations etc including: train_dataloader, batch_size, creating the optimizers and schedulers (from function) etc
            # NOTE: The default optimizer is "reasonable" but this func can be overridden or an alternative passed in args to Trainer.
            # NOTE: it looks like these settings match pytorch.optim defaults which are the same in Trainer.
            ########### MTB ############
        ...
        if accuracy_per_epoch[-1] > best_pred:
            ...
            torch.save({
                ...
                }, os.path.join("./Vault/mtb/output/" , "test_model_best.pth.tar"))

        if (epoch % 1) == 0:
            save_as_pickle("test_losses_per_epoch.pkl", losses_per_epoch)
            ...
            torch.save({
                    ...
                    'scheduler' : scheduler.state_dict(),\
                    'amp': amp.state_dict() if amp is not None else amp
                }, os.path.join("./Vault/mtb/output/" , "test_checkpoint.pth.tar"))
    ...
    plt.savefig(os.path.join("./Vault/mtb/output/" ,"loss_vs_epoch.png"))
    ...
    plt.savefig(os.path.join("./Vault/mtb/output/" ,"accuracy_vs_epoch.png"))
